...
from d2c_scrapling_enricher import ScraplingPageEnricher
...
class BraveSearchCollector:
    ...
    def __init__(self, api_key: str, config: dict):
        ...
        self.page_enricher = ScraplingPageEnricher.from_env(config, logger=logger)
    ...
    def build_record(
        ...
        enrichment = self.page_enricher.enrich(
            url,
            country=country,
        ...
        enriched_title = enrichment.get("page_title", "").strip() or title
        enriched_quote = enrichment.get("quote_original", "").strip() or snippet[:500]
        enriched_currency = enrichment.get("currency", "").strip()
        ...
        record = {
            ...
            "value": enriched_title[:300],
            "currency": enriched_currency,
            "quote_original": enriched_quote[:500] if enriched_quote else "",
            "source_url": url,
            "source": domain,
        ...
        if enrichment:
            record.update(
                {
                    "fetch_method": enrichment.get("fetch_method", ""),
                    "page_title": enrichment.get("page_title", "")[:300],
                    "published_at": enrichment.get("published_at", "")[:64],
                    "price_evidence": enrichment.get("price_evidence", "")[:120],
                }
            )
        return record
...
def main():
    ...
    logger.info(f"Output: {output_path}")
    logger.info(f"Scrapling enrichment summary: {collector.page_enricher.summary()}")
